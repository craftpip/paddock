...
- **Root-owned instance data breaks delete/reset — the wipe ALWAYS runs with
  root access.** Agent containers write their data dir as root; the webui runs
  as uid 1000 and a plain `fs.rmSync` throws `EACCES`. `removeInstanceDir`
  therefore always wipes through a one-shot root helper container of our own
  image (host-path bind). The helper clears the mount CONTENTS (`find /d
  -mindepth 1 -delete`) — never `rm -rf /d`, which fails on a bind mountpoint
  ("Device or resource busy"). If a delete/reset still reports the
