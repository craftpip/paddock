...
- **Root-owned instance data breaks delete AND reset.** Agent containers write
  their data dir as root; the webui runs as uid 1000 and a plain `fs.rmSync`
  then throws `EACCES`. `removeInstanceDir` catches it and finishes via a
  one-shot root helper container of our own image (host-path bind). The helper
  clears the mount CONTENTS (`find /d -mindepth 1 -delete`) — never `rm -rf
  /d`, which fails on a bind mountpoint ("Device or resource busy"). If a
  delete/reset still reports the `chown -R 1000:1000 ...` hint, run it on the
  host. Never `docker exec paddock node …` on DB paths (recreates root-owned
  `app.db*`); the webui image runs root by default, which is why the helper
  works.
