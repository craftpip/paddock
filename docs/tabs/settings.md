...
Name, agent type, runtime, status, and image. Image comes from the `AGENT_IMAGES[agent_type]` map in `vm-manager.js` (`paddock-vm-openclaw:latest` etc.).
...
- Guard rail: if the image has no docker CLI (`docker exec <name> sh -lc 'command -v docker'` fails while the container runs), the toggle errors with "The image has no docker CLI — rebuild the image to enable docker access". The openclaw/picoclaw/nanobot/hermes images don't ship the CLI today, so enabling on a running agent errors until the image is rebuilt.
...
| GET | `/api/agents/:name/settings` | `{ allowDocker, network, image }` from `meta.env` (`DOCKER`, `NETWORK`) + `AGENT_IMAGES` |
...
- Exports `AGENT_IMAGES` and `AGENT_BUILD_REL`
